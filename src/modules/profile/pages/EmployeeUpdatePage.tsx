...
const EmployeeUpdatePage: React.FC = () => {
  ...
  const validateFormData = () => {
    ...
    if (
      ...
      !cccdRegex.test(formData.identityCardNumber)
    ) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const req: UpdateProfileRequestForHR =
        UpdateProfileRequestForHR.fromUser(formData);
      const updatedUser = await profileApi.updateProfileForHR(
        formData.userId,
        req
      );
